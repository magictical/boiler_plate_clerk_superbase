...
- [ ] `app/onboarding/gym-create/page.tsx` 생성 (암장 이름, 색상 추가, 6단계 티어 박스, 드래그 영역, 저장 버튼)
- [ ] `components/onboarding/GymCreator.tsx` (색상 추가/삭제, ColorPicker, dnd-kit, 티어 매핑 시각화)
- [ ] `actions/gyms.ts`에 `createGymWithScales(gymData, scales[])` 추가
- [ ] 저장 성공 시 홈짐 설정 후 `/onboarding/tier-assign` 이동
